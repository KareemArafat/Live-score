...
class LineupsMember {
  ...
  factory LineupsMember.fromJson(Map<String, dynamic> json) {
    return LineupsMember(
      status: json["status"],
  ...
  LineupMemberEntity toEntity(Map<int, MemberEntity> members) {
    return LineupMemberEntity(
      lineupPlayerId: id!,
      lineupCountryId: nationalId!,
      teamId: competitorId!,
      playerId: members[id]!.playerId,
      playerName: members[id]?.playerName ?? 'Un Known',
      playerShortName: members[id]?.playerShortName ?? 'Un Known',
